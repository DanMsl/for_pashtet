...
RectangleWindow::RectangleWindow(QDialog *parent) : QDialog(parent) {
    ...
    connect(button_line_color, SIGNAL(clicked()), this, SLOT(set_color_line()));
    connect(button_fill_color, SIGNAL(clicked()), this, SLOT(set_color_fill()));
    connect(button_ok, SIGNAL(clicked()), this, SLOT(do_rectangle()));
}
...
void RectangleWindow::set_color_line() {
    *color_line = QColorDialog::getColor();
}


void RectangleWindow::set_color_fill() {
    *color_fill = QColorDialog::getColor();
}


void RectangleWindow::do_rectangle() {
    bool isFill = false;
    if (fill->isChecked())
        isFill = true;
    emit do_rectangle(width_line_input->value(), isFill, *color_line, *color_fill);
}
